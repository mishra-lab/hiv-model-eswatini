...
class Slicer():
  ...
  def __str__(self):
    return 'Slicer: {} {{{}}} [{}]'.format(self.key,dictstr(self.pop),self.label)

  def __repr__(self):
    return 'S: {} {{{}}}'.format(self.key,dictstr(self.pop))
...
slicers = {Si.key:Si for Si in [
  ...
  Slicer('WM',    dict(s=(0),  i=(1)),      (.80,.60,.60),'Medium Risk Women'),
  ...
  Slicer('LT',    dict(p=(0)),              (1.0,.85,00),'Long Term'),
  Slicer('ST',    dict(p=(1)),              (1.0,.60,.00),'Short Term'),
  Slicer('SWR',   dict(p=(2)),              (1.0,.35,.00),'Sex Work Regular'),
  Slicer('SWO',   dict(p=(3)),              (1.0,.10,.00),'Sex Work Occasional'),
]}
