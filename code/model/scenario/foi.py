...
def run_ep():
  ...
  ekwds = dict(tvec=tvec['main'],t=tvec['plot'],
    snames=['all','w','m','aq','fsw','cli'],onames=['incidence','prevalence'])
  Ps = fio.load(fname('npy','fit','Ps',case='base'))
  for case in cases:
    log(1,case)
    R1s = system.run_n(dict_list_update(Ps,foi_mode=case),t=tvec['main'])
    E = out.expo(R1s,**ekwds)
    fio.save_csv(fname('csv','foi-ep','wiw',case=case),out.wiw(R1s,tvec['main'],tvec['plot']))
    if case == 'base':
      R2s = copy(R1s)
    else:
      EAD = out.expo(R1s,R2s=R2s,vsop='1-2',  **ekwds)
      ERD = out.expo(R1s,R2s=R2s,vsop='1-2/2',**ekwds)
