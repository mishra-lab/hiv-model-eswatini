...
def run():
  ...
  fio.save_csv(fname('csv','cal','ll'),{'seed':seeds,'ll':[R['ll'] if R else np.nan for R in R0s]})
  ...
  fit.plot_cal(tvec['cal'],R0s,T,fname=fname('fig','cal','plot'))
...
def merge():
  ...
  for b in range(N['batch']):
    for ll in fio.load_csv(fname('csv','cal','ll',b=b),fmt='dict'):
      P0s[seeds.index(int(ll['seed']))].update(ll=ll['ll'])
  fio.save_csv(fname('csv','cal','Ps',b='all'),P0s)

def rerun():
  log(0,'scenario.calibrate.rerun')
  T = target.get_all_esw()
  Ps = fio.load(fname('npy','fit','Ps',b='all'))
  Rs = system.run_n(Ps,t=tvec['main'],T=T)
  # fit.plot_cal(tvec['main'],Rs,T,fname=fname('fig','fit','plot',b='all'))
  fio.save_csv(fname('csv','fit','infs',case='base',b='all'),
