...
%     and tried to tie in inequalities again -- what do you think? %SM: nice
...
% JK: We did, but might not mention in the abstract %SM: agree (for reasons you noted)
...
Taking observed cascade scale-up in Eswatini as the base-case %SM: nice (this seems clear to me re: SS comments around base-case, esp with the achieved 95-95-95 among FSW in the background above)
% SM: nice and clear
% JK: thanks!
...
% JK: not sure what you mean? 60-40-80 was not observed but defined for the scenario  %SM: I think she means the left-behind-cascade X-X-X (not observed). Do we (or can we) make room for these? but ok if no room (and i have a feelling will not have enough room).
We quantified relative additional cumulative HIV infections by 2030 %SM: trick in abstracts is that if need word count, cutting these joiner words like "then/next/further, etc." (although important for flow when more text allowed) often helps.
...
%     After rephrasing the first part, maybe it is more clear? %SM: yes
...
% JK: I am really liking the 'inequalities' framing throughout %SM: :) 
can undermine the anticipated prevention impacts of cascade scale-up.
Identifying and addressing inequalities in the ART cascade, as evident among FSW in Eswatini, %SM: if need word count, could wordsmith and cut out "identifing and".
...
% JK: not sure what you mean? %SM: something like edit above?
...
%     and very tight on words, so maybe remove this? %SM: agree
\paragraph{Funding}
