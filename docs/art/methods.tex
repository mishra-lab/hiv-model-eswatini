...
%     (e.g. % initiated after 12 months) may/not support deriving a relative rate  %SM: yes, same here - my recollection is that once on ART, less differnetial re: adherence and conditinal VLS, but there are some studies (like in South Africa with Siyambaphili) where I think conditional VLS was lower among FSW. For this study/paper, I think we are ok as is and revise if/as reviewer may ask/suggest later.
...
% JK: Good point -- in next calibration we can explore <1 for FSW and/or clients too  %SM: agree 
...
%     maybe just highlight the tables which summarize these targets? %SM:agree
...
%     & introduced the 4 cases first before diving into the details as you suggest LW  %SM: this was a nice suggestion - reads well
...
% JK: hopefully clarified? %SM: yes to me
...
%     so I might leave it out of the discussion? SM: agree with leave out
...
%     though I agree, it has not been defined until here... %SM: was introduction, so feel like its ok. (can see if reviewer also  needs it to be repeated, but ok as is I think)
...
%     hopefully moving the 'as follows' here helps? %SM: yes
...
% JK: I've changed 'unsuppression' -> 'non-suppression', which is maybe slightly better? %SM: yes, I like that on re-reading it...(and have seen similar re: not suppressed in other papers)
...
%     due to non-suppression among subpop i in the context of C_j', is more intuitive? %SM: yes, agree ... it is more intuitive and flows more logically re: central thesis of paper/study
...
%     not sure what other prevalence ratios we would mean? %SM: sorry, I see it now!
...
%     as it might just add repetition / confusion? %SM: I think the layered approaach now is good
