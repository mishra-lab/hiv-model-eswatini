...
In this section, we propose such an approach: %SM: not cirtical, but would it be accurate to say "develop and propose"? since you are walking through its develpment as well in paras below?
...
% TODO: (?) mention by name pair-based and individual-based models here %SM: agree
%===================================================================================================
\subsection{Conceptual Development}\label{foi.prop.concept} %SM: LOVE this section
...
The issue of multiple post-transmission partnerships is discussed in \sref{foi.prop.mp}. %SM: multiple or concurrent? or means same thing really?
\begin{figure}
...
we do not define any per-partnership probability of transmission $B$. %SM: should it be "we need not" isntad of "we do not"?
...
% TODO: (*) double-check that this reduction should be propagated through the mixing patterns %SM: not sure I follow what this means re: propogated through the mixing patterns? i think we mean the effective reduction is propogated (like herd effects essentially?) but that the actual partnersips are not influenced by whether transmissions occurred or not (i.e. we assume no behavoiur change following transmission?)
...
all possible combinations of partnership states in pair-based models \cite{Kretzschmar2017}. %SM: nice
...
We call these partnerships ``transmission-driven seroconcordant''  %SM: reallly like this
