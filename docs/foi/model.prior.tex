\section{Current Models of the Force of Infection}
...
\begin{figure}
  ...
  \caption{Probability of transmission $B$ vs number of contacts (sex acts) $A$.
    ...
    some contacts in all partnerships (red) from \eqref{eq:B.m.scap} vs
    all contacts in some partnerships (blue) from \eqref{eq:B.m.acsp};
...
and the binomial adjustment has almost no effect over 1 year, $B(A\mid\delta=1) \approx \beta A$.
...
For completeness, and since it will be useful later, we define a final model \case{3} as:
...
In the middle, models \case{1b} and \case{2} include
...
\par
A final limitation affecting all models \case{1-3} is that:
