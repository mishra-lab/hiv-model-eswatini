...
Such data were later explained by heterogeneity in  %SM: not sure I followed this?
...
As this heterogeneity was quantified \cite{Gray2001} %SM: which heterogeneity? suggest indicating what "this" might refer to?
and incorporated into HIV transmission models \cite{Moghadas2003},
the probability of transmission was increasingly parameterized per act \vs per partnership. %SM: suggest instead of "vs" to use "instead of"... here
\par
The shift to per-act \vs per-partnership parameterization highlighted %SM: very nicely explained para
...
\par
Thus, over the last X years, different force of infection equations %SM: if have, suggest including number of years, or even last  decades, or something like that...?
...
\end{equation}
Since transmission of HIV can only occur once, %SM: should the term be "acquistion of HIV can only occur once"?
...
should be driven by the factor(s) in question. %SM: nice
...
still corresponds to a single average partnership. %SM: nice i like how you sequentially linked explanations...
...
\begin{itemize}
  ...
  \end{equation}
  % TODO: (?) describe why this looks like ~ wph even though partnerships are 'distinct'  %SM: I think would be worth doing, but not critical and would also be ok if we await reviewer to ask. but expect reviewer might ask...
...
\emph{over a given time period $\Delta_t$}. %SM: nice
Since most models are now solved using numerical computation (rather than solved analytically),
this period $\Delta_t$ could be matched to the timestep of the numerical solver.%
\footnote{\label{foot:ode}%
  Popular numerical solvers include: %SM: nice :) 
...
% JK: I don't think the above example is helpful. %SM: agree
...
because the proportion of inert sex acts is effectively underestimated. %SM: nice
...
%---------------------------------------------------------------------------------------------------
\paragraph{Aggregating Past/Future Sex Acts} %SM: really like this section
